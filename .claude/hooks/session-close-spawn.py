...
cold-inferred decisions, so the owner can veto a wrong block in decisions.md.
...
def surface_notice(state):
    ...
    for entry in last["logged"]:
        ...
        print(f"Session-close sweeper auto-logged to records/decisions.md from "
              f"session {entry.get('session')}: {titles}. Tell the owner in one "
              f"line; they can veto by editing the block.")
    last["noted"] = True
    try:
