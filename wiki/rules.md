...
- **Boilerplate, binding on every operation:** write only inside `wiki/` (triage's one exception: Retire moves the page to `wiki/annex/`); no tracker, chat, or external writes; no git commit unless the owner asks; scratch only under repo-root `.tmp/`; deleting any wiki page takes the owner's explicit yes; an empty checkpoint is still reported honestly, never skipped.
