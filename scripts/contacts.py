...
def _normalize(data, slug):
    out = blank_contact(slug, data.get("name") or slug)
    for k in FIELDS:
        if k not in data:
            continue
        v = data[k]
        if k in LIST_FIELDS:
            ...
            out[k] = "" if isinstance(v, (list, dict)) else str(v)
    out["_missing"] = [k for k in FIELDS if k not in data]
    return out


def _read_jsonl(path):
    """Every JSON-object line in a jsonl file, in file order. Unparseable or
...
def load_contacts(root=None):
    """{slug: contact dict} for every row in `contacts/contacts.jsonl`."""
    out = {}
    for data in _read_jsonl(contacts_file(root)):
...
def load_not_names(root=None):
    ...
    """
    out = []
    for row in _read_jsonl(not_names_file(root)):
...
def validate(root=None):
    ...
    for slug, c in sorted(store.items()):
        ...
        if not c["slug"]:
            violations.append(f"{f}: empty slug")
        ...
        if c["slug"] in seen_slug:
            violations.append(f"{f}: duplicate slug, also in {seen_slug[c['slug']]}")
        seen_slug[c["slug"]] = f
        if c["wiki"]:
    ...
    names = {c["name"].strip().lower(): slug for slug, c in store.items()}
...
def cmd_add(args):
    slug = args.slug
    if slug in load_contacts(args.root):
        print(f"refusing: contact '{slug}' already exists")
        return 1
    c = blank_contact(slug, args.name, args.status, args.affiliation or "",
                      args.role or "")
    save_contact(c, args.root)
    print(f"wrote contact '{slug}' to contacts/{CONTACTS_FILE}")
    return 0
