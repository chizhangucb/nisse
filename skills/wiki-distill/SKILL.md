...
4. **Write, after the yes:** insert Evidence bullets in date order on each affected page, create the approved new pages (with inbound links), fill each source's `# Distilled` section, stamp `distilled:` to today, mint approved tags into the registry, append the pass to `wiki/metadata/log.jsonl` via `scripts/aios_ledger.py append-log`.
