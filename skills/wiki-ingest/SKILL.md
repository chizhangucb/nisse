...
Batch-native landing pass: **enumerate → acquire → mirror → source page → metadata → report**. Read `wiki/CLAUDE.md`, `wiki/rules.md`, and `references/ingest-rules.md` first, in full; precedence: schema > shared rules > skill rules > this file.
...
6. **Update metadata:** monthly index line (meetings) or `index.md` (non-meetings); append to `wiki/metadata/log.md`.
